class NeuralNetwork {
  ...
  NeuralNetwork(float [] LowerWeight, float [] HigherWeight, float [] LowerBias, float [] HigherBias){
    for(int i=0; i < weights.length-1; i++){
      weights[i] = random(LowerWeight[i], HigherWeight[i]);
    }
    for(int i=0; i < biases.length-1; i++){
      biases[i] = random(LowerBias[i], HigherBias[i]);
    }    
  }
